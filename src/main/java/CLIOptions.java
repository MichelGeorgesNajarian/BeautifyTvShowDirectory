...
//		this.longNameMatchings.put("--move", 'm');
		...
		this.longNameMatchings.put("--verbose", 'v');
					...
					try {
						if (currOpt.getNumOpt() == 1) { // only do this if there is exactly 1 argument to pass for this option
							...
							if (temp.length > 1) {
								...
								if (Boolean.getBoolean(temp[1])) this.numOptTrue++;
							}							
						} else {
...
//		}
	...
	public Opts matchOpt(char arg) throws InstanceNotFoundException {
		...
		throw new InstanceNotFoundException(); // options to match was not found
	}
	...
	public void printHelpPage() {
		System.out.printf("Usage: BeautifyTvShow [--Options -o --option=true --option=false --opt=param1,param2,param3] directory1/ directory2/ directory3/\n"
				...
				+ "\t--log / -l: log all the changes done in file BEAUTIFY.LOG in the current directory\n"
//				+ "\t--move / -m: do NOT keep the old file, just the new file\n"
				...
				+ "\t--verbose / -v: print on screen updates as Beautification is happening\n"
	...
	/**
	 * @return the numOptTrue
	 */
	public int getNumOptTrue() {
		return numOptTrue;
	}
	/**
	 * @param numOptTrue the numOptTrue to set
	 */
	public void setNumOptTrue(int numOptTrue) {
		this.numOptTrue = numOptTrue;
	}
